  ...
  fetch("/api/data")
    ...
    .then(data => {
      ...
      if (data.other_crypto) {
        // Üst barda sadece fiyat gösterilecek şekilde detay objesinden fiyatları al
        combinedRates.BTCUSD = data.other_crypto.bitcoin?.price_try ?? combinedRates.BTCUSD;
        combinedRates.ETHUSD = data.other_crypto.ethereum?.price_try ?? combinedRates.ETHUSD;
        combinedRates.XRPUSD = data.other_crypto.ripple?.price_try ?? combinedRates.XRPUSD;
      }
